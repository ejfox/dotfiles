...
> - **Quarters (added 2026-07-06):** `⇧h/⇧l` = quarter on that side (first press = TOP quarter), pressing the same key again **toggles** to the other quarter on that side (⇧h: top-left ⇄ bottom-left). `⇧k/⇧j` = top/bottom quarters, repeat toggles left⇄right corner. `⇧space` = **golden-ratio centered float** (1/φ ≈ 61.8% of screen; again = 1/φ² ≈ 38.2%). `u` = **undo last snap** (single-depth, Hammerspoon `windowSnapUndo()`). Shift manipulators sit BEFORE the bare ones in the rule (bare keys match `optional: any`, first match wins).
> - **Cheatsheet (added 2026-07-15):** `?` = toggle a floating translucent cheatsheet panel (`cheatsheetToggle()` in init.lua, renders `~/.dotfiles/.config/cheatsheet.html`; drag by top edge, frame persists).
