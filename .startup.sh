...
fetch_tasks() {
  ...
  # things.sh format: "Area|Task|url" - extract just task names, strip emojis
  local tasks=$(things.sh today 2>/dev/null | cut -d'|' -f2 | \
    sed 's/^[^[:alnum:]]*//' | head -5)
  [ -z "$tasks" ] && return 0
...
fetch_calendar() {
  ...
  # Strip ALL ANSI codes and non-ASCII, clean up whitespace
  icalBuddy -f -nc -nrd -iep "datetime,title" -po "datetime,title" -tf "%H:%M" -df "" -b "" \
    -npn -n eventsToday 2>/dev/null | \
...
fetch_mirror_data() {
  ...
  # RescueTime (30 min cache) - fast endpoint, 2s is fine
  if ! cache_fresh "$CACHE_DIR/rescuetime.json" 30; then
    (timeout 2.0 curl -fsSL https://ejfox.com/api/rescuetime 2>/dev/null > "$CACHE_DIR/rescuetime.json.tmp" && \
      [ -s "$CACHE_DIR/rescuetime.json.tmp" ] && mv "$CACHE_DIR/rescuetime.json.tmp" "$CACHE_DIR/rescuetime.json") &
  fi

  # GitHub (30 min cache) - needs longer timeout, big payload
  if ! cache_fresh "$CACHE_DIR/github.json" 30; then
    (timeout 5.0 curl -fsSL https://ejfox.com/api/github 2>/dev/null > "$CACHE_DIR/github.json.tmp" && \
      [ -s "$CACHE_DIR/github.json.tmp" ] && mv "$CACHE_DIR/github.json.tmp" "$CACHE_DIR/github.json") &
  fi
}
...
################################################################################
# CIPHER DAILY - Morning guidance (generated by launchd at 7am)
################################################################################
